    ...
    return (
        <nav className="bg-white shadow-md fixed w-full top-0 left-0 z-50">
            <div className="container mx-auto px-6 py-4 flex justify-between items-center">
                {/* Logo */}
                <h1 className="text-2xl font-extrabold text-red-500 flex items-center gap-2 cursor-pointer" onClick={() => {
                    navigate("/")
                }}>
                ...
                <ul className="hidden md:flex">
                    ...
                    {getUserInfo("role_id") === 1 &&
                        <NavItem to="/admin-panel" icon={FiUser}>Admin</NavItem>
                    }
                    <NavItem to="/upload" icon={FiUpload}>Upload</NavItem>
                </ul>
            ...
            {/* Mobile Menu */}
            <div
                className={`md:hidden absolute top-16 left-0 w-full bg-white shadow-md transition-all duration-300 z-50 ${isOpen ? "block opacity-100" : "hidden opacity-0"
                    }`}
            >
